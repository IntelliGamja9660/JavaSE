 ...
 *          3) speed - 5
 *          
 *      기능	1) 실명 - 상대 챔프 침묵 + 독 데미지  메서드
 *      	2) 신속한 이동 - 3초간 speed + 2
 *      
 *      클래스는 객체가 아니다!!!
 *      
 * 클래스(Class)
 *   객체를 생성하기 위한 제작 설명서이다.
 *   Class로 작성된 코드를 읽어 인스턴스화(객체화) -> 메모리에 저장 또는 load 
 *   - 붕어빵 틀, 와플 기계
 *   
 * 클래스 구조
 * [접근 제한자] class 클래스명 [extends 부모클래스] [implement 인터페이스] {
 * 		
 * 		[생성자] => 초기화
 * 		변수(멤버변수, 인스턴스 변수, 필드) => 속성
 * 		메서드(멤버메서드, 인스턴스 메서드) => 기능
 * 	}
 * 
 ...
 *   자바에서 동적으로 할당된 메모리(heap) 중에서 더 이상 사용되지 않는 메모리를 해제하는 기능을 수행하는 프로그램
 *      	
 * 
 */
